...
docGet =
  makeTests "Get local document size and revision"
    [ withDb $ testAgainstFailure "No information for non-existent doc" (Local.get docGetDoc "foo" Nothing) NotFound
...
docPut =
  makeTests "Create and update a local document"
    ...
    , withDb $ testAgainstSchema
                 ...
                 (\c -> do
                    res <- Local.put docPutParam "foo" Nothing testDoc c
                    let (Right (rev, _)) = getKey "rev" res
                    Local.put docPutParam "foo" rev testDoc c)
                 "put--db-docid.json"
    ]
  ...
  makeTests "Delete a local document"
    ...
    , withDb $ testAgainstSchema
                 ...
                 (\c -> do
                    res <- Local.put docPutParam "foo" Nothing testDoc c
                    let (Right (rev, _)) = getKey "rev" res
                    Local.delete docPutParam "foo" rev c)
